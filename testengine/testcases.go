...
var TestCases = []*TestCase{
	...
	{"VNFBPT07", true, "Found setuid binaries cannot be writable by non-root users.", []*Dependency{{Id: "ENUM03", Type: Stdout, VarName: "setuid_binaries"}}, []string{sh}, true, `(for b in $setuid_binaries; do [ -x "$b" ] && [ -w "$b" ] && echo "$b" ;done)`, nil, PassedWhenStdoutEmpty},
	{"VNFBPT08", true, "Found setgid binaries cannot be writable by non-root users.", []*Dependency{{Id: "ENUM04", Type: Stdout, VarName: "setgid_binaries"}}, []string{sh}, true, `(for b in $setgid_binaries; do [ -x "$b" ] && [ -w "$b" ] && echo "$b" ;done)`, nil, PassedWhenStdoutEmpty},
	...
	{"VNFBPT11", true, "Critical files cannot be writable by a non-root user", []*Dependency{{Id: "ENUM02", Type: Stdout, VarName: "user_writable"}}, []string{sh}, true, "critical_writable=" + critical_writable + `;for uw in $user_writable; do [ -f "$uw" ] && IFS=$'\n'; for cw in ${critical_writable}; do [ "$cw" = "$uw" ] && [ -w "$cw" ] && ls -l $cw; done ; done`, nil, PassedWhenStdoutEmpty},
	{"VNFBPT12", true, "Critical directories cannot be writable by a non-root user", []*Dependency{{Id: "ENUM02", Type: Stdout, VarName: "user_writable"}}, []string{sh}, true, "critical_writable=" + critical_writable_dirs + `;for uw in $user_writable; do [ -d "$uw" ] && IFS=$'\n'; for cw in ${critical_writable_dirs}; do [ "$cw" = "$uw" ] && [ -w "$cw" ] && ls -ld $cw; done ; done`, nil, PassedWhenStdoutEmpty},
	...
	{"VNFBPT16", true, "Other users' SSH files must not be readable", nil, []string{sh, find}, true, find_opts + `find / $find_opts \( -name "*id_dsa*" -o -name "*id_rsa*" -o -name "*id_ecdsa*" -o -name "*id_ed25519*" -o -name "known_hosts" -o -name "authorized_hosts" -o -name "authorized_keys" \) -readable ! -user $USER -exec ls -la {} \; 2>/dev/null`, nil, PassedWhenStdoutEmpty},
	...
	{"VNFBPT30", true, "A non-root user must not be able to write to any service unit file", []*Dependency{{Id: "ENUM02", Type: Stdout, VarName: "user_writable"}}, []string{sh, grep}, true, `printf "$user_writable\n" | grep -E "^/etc/(init/|init\.d/|rc\.d/|rc[0-9S]\.d/|rc\.local|inetd\.conf|xinetd\.conf|xinetd\.d/)"`, nil, PassedWhenStdoutEmpty},
	{"VNFBPT31", true, "A non-root user must not be able to write to any executable used by service", nil, []string{sh, grep, tr, sort}, true, `for b in $(grep -ERvh "^#" /etc/inetd.conf /etc/xinetd.conf /etc/xinetd.d/ /etc/init.d/ /etc/rc* 2>/dev/null | tr -s "[[:space:]]" "\n" | grep -E "^/" | grep -Ev "^/(dev|run|sys|proc|tmp)/" | sort -u); do [ -x "$b" ] && [ -w "$b" ] && echo "$b" ;done`, nil, PassedWhenStdoutEmpty},
	{"VNFBPT32", true, "A non-root user must not be able to write to any systemd files", []*Dependency{{Id: "ENUM02", Type: Stdout, VarName: "user_writable"}}, []string{sh, grep}, true, `printf "$user_writable\n" | grep -E "^/(etc/systemd/|lib/systemd/).+\.service$" 2>/dev/null`, nil, PassedWhenStdoutEmpty},
	{"VNFBPT33", true, "A non-root user must not be able to write to any executable used by systemd services", nil, []string{sh, grep, tr, sort}, true, `for b in $(grep -ERh "^Exec" /etc/systemd/ /lib/systemd/ 2>/dev/null | tr "=" "\n" | tr -s "[[:space:]]" "\n" | grep -E "^/" | grep -Ev "^/(dev|run|sys|proc|tmp)/" | sort -u); do [ -x "$b" ] && [ -w "$b" ] && echo "$b" ;done 2>/dev/null`, nil, PassedWhenStdoutEmpty},
	...
	{"VNFBPT48", true, "Processes must not be invoked with secrets/credentials", []*Dependency{{Id: "ENUM05", Type: TestFunc, VarName: ""}}, []string{sh, grep, groups}, true, ``, vnfbpt48, PassedWhenRetCodeSuccessful},
	...
	{"VNFBPT51", true, "A non-root user must not have access permissions to .htppasswd files", nil, []string{sh, find}, true, find_opts + `find / $find_opts -name "*.htpasswd" -print -exec cat {} +`, nil, PassedWhenStdoutEmpty},
